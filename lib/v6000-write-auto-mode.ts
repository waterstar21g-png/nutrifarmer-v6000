...
import { V6000_AUTO_MODE_KEY, V7000_AUTO_MODE_KEY } from '@/lib/v6000-write-config';

export function readAutoMode(): boolean {
  if (typeof window === 'undefined') return false;
  const v =
    window.localStorage.getItem(V6000_AUTO_MODE_KEY) ??
    window.localStorage.getItem(V7000_AUTO_MODE_KEY);
  return v === '1';
}
