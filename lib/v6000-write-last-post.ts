...
import { V6000_LAST_POST_KEY, V7000_LAST_POST_KEY } from '@/lib/v6000-write-config';
...
export function readLastPost(): V6000LastPost | null {
  ...
  try {
    const raw =
      window.localStorage.getItem(V6000_LAST_POST_KEY) ??
      window.localStorage.getItem(V7000_LAST_POST_KEY);
    if (!raw) return null;
