...
export async function POST(req: NextRequest) {
  ...
  if (!keyword || !productName) {
    ...
    if (!body.skipVision) {
      ...
      try {
        ...
        console.error('[scout/analyze] vision', msg);
        return NextResponse.json(
  ...
  try {
    ...
    return NextResponse.json({
      ok: true,
      vision: visionConfidence != null ? { confidence: visionConfidence, keyword, productName, category } : undefined,
      scout,
    });
  } catch (e) {
    const msg = e instanceof Error ? e.message : 'scout_failed';
    console.error('[scout/analyze] itemscout', msg);
